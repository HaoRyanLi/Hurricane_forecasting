...
Train_data = all_data[:1,:num_train]
...
Test_data = all_data[:1, num_train:]
...
class TrainerModule:
    ...
    def create_functions(self):
        ...
        def squential_loss(i, args):
            ...
            outs = self.model.apply({'params': params, 'batch_stats': batch_stats}, 
                                        u_ml, self.train_hparams['dt'], self.f_cori, train=True, mutable=['batch_stats'])
        ...
        def forward_map(i, args):
            ...
            u_ml_out = self.model.apply({'params': state.params, 'batch_stats': state.batch_stats}, 
                                        u, self.train_hparams['dt'], self.f_cori, train=False)
    ...
    def init_model(self, exmp_inputs):
        ...
        variables = self.model.init(init_rng, exmp_inputs, self.train_hparams['dt'], f_cori=jnp.zeros((Ny_int, Nx_int, 1)), train=False)
        self.init_params, self.init_batch_stats = variables['params'], variables['batch_stats']
        self.state = None
        self.xy = jnp.concatenate((xx_norm,yy_norm), axis=-1)
        self.f_cori = 2*Omega*jnp.sin(yy*rad)
        self.f_cori = jnp.expand_dims(self.f_cori, axis=0)

        # Intialize test setting
        batch_size_test, Nt_test, H, W, C = Test_data.shape
        self.test_data = Test_data

        self.test_xy = jnp.tile(self.xy, (batch_size_test,1,1,1))
        print(f"The shape of f_cori: {self.f_cori.shape}")
    ...
    def train_model(self, num_epochs=1000):
        ...
        for epoch_idx in tqdm(range(1, num_epochs+1)):
            ...
            err_test = self.eval_model(self.state, self.test_data)
            t2 = time.time()
...
trainer = TrainerModule(model_name="UNet", model_class=U_net.UNet,
                        model_hparams={"act_fn": nn.relu, "act_fn_name": 'relu',  "padding": "REPLICATE", "block_size": (4, 8, 16, 32, 64, 128, 128), 
                        "out_features": Train_data.shape[-1]-2, "model_type": "U_net_modified", "Nc_uv": Nc_dim}, 
